...
$status="0";
$sql = "UPDATE o_collector SET Status=:status WHERE  id=:eid";
...
$sql = "UPDATE o_collector SET Status=:status WHERE  id=:aeid";
$query = $dbh->prepare($sql);
$query -> bindParam(':status',$status, PDO::PARAM_STR);
...
if(isset($_REQUEST['del']))
	{
$did=intval($_GET['del']);
$sql = "delete from o_collector WHERE  id=:did";
$query = $dbh->prepare($sql);
...
<?php $sql = "SELECT * from  o_collector ";
$query = $dbh -> prepare($sql);
$query->execute();
